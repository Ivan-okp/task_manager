...
- Запуск сервера локально: uvicorn src.task_manager.main:app --reload
...
2. Переменные окружения
   Для работы приложения и подключения к базе данных используются переменные окружения. Это позволяет безопасно хранить секретные данные и менять настройки без изменения кода.
